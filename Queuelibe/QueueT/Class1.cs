...
namespace QueueT
{
    namespace QueueCalc
    {
        public class ClacQueueT
        {
            ...
            public void CalcMd1(ref double p0, ref double pn, ref double ls, ref double lq, ref double ws, ref double wq, int s, int n)
            {
                p0 = 1 - rou;
                //pn = ;
                lq = Math.Pow(rou, 2) / (2 * (1 - rou));
                ls = lq + rou;
                ws = ls / lam;
                wq = lq / lam;
            }
            public void CalcMek1(ref double p0, ref double pn, ref double ls, ref double lq, ref double ws, ref double wq, int s, int n)
            {
                int k = 1;
                p0 = 1 - rou;
                //pn =;
                lq = (1 + k) * Math.Pow(lam, 2) / (2 * k * meu * (meu - lam));
                ls = (1 + k) * lam / (2 * k * meu * (meu - lam));
                ws = ls / lam;
                wq = lq / lam;
            }
            #endregion
